...
function renderLive() {
  ...
  const exHtml = live.exercises.map((ex, i) => {
    ...
    const setPills = ex.sets.map((s, j) => `<div class="set-pill-logged" data-edit-set="${i}-${j}"><span class="w">${s.w}</span><span class="x">×</span>${s.r}<span class="rm" data-rm-set="${i}-${j}">×</span></div>`).join('');
    const targetHtml = sug ? `<div class="live-ex-target">${sug.rec}</div>` : '';
    const lastW = ex.sets.length ? ex.sets[ex.sets.length - 1].w : '';
    return `
      <div class="live-ex ${isCurrent ? 'current' : ''}" data-ex-idx="${i}" draggable="true">
        <div class="live-ex-head">
          ...
          <button class="live-ex-del" data-del-ex="${i}" title="Remove exercise">✕</button>
        </div>
        ${targetHtml}
        ${setPills ? `<div class="set-list">${setPills}</div>` : ''}
        <div class="set-add">
          <input type="number" placeholder="weight" data-w-idx="${i}" inputmode="decimal" step="0.5" value="${lastW}" />
    ...
    <div class="live-active">
      <div class="live-header">
        <div>
          ...
          <button class="live-rename" id="live-rename">✎ Rename</button>
        </div>
  ...
  // Tap a logged set to edit its weight/reps inline
  document.querySelectorAll('[data-edit-set]').forEach(pill => {
    pill.addEventListener('click', (e) => {
      if (e.target.closest('.rm')) return;
      e.stopPropagation();
      const [i, j] = pill.dataset.editSet.split('-').map(Number);
      editSetInline(pill, i, j);
    });
  });
  ...
  document.querySelectorAll('.live-ex').forEach(el => {
    el.addEventListener('click', (e) => {
      if (e.target.closest('input') || e.target.closest('button') || e.target.closest('.rm') ||
          e.target.closest('.drag-handle') || e.target.closest('[data-edit-set]')) return;
      live.currentExIdx = +el.dataset.exIdx;
      saveLive();
      renderLive();
    });
  });
  // Delete an exercise from the live session
  document.querySelectorAll('[data-del-ex]').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      const i = +btn.dataset.delEx;
      const ex = live.exercises[i];
      if (ex.sets.length > 0 && !confirm(`Remove "${ex.name}" and its ${ex.sets.length} logged set${ex.sets.length > 1 ? 's' : ''}?`)) return;
      live.exercises.splice(i, 1);
      if (live.currentExIdx >= live.exercises.length) live.currentExIdx = Math.max(0, live.exercises.length - 1);
      saveLive();
      renderLive();
    });
  });
  document.getElementById('live-rename').addEventListener('click', () => {
    const name = prompt('Session name:', live.name);
    if (name && name.trim()) {
      live.name = name.trim();
      saveLive();
      renderLive();
...
// Inline edit a logged set
function editSetInline(pill, exIdx, setIdx) {
  const s = live.exercises[exIdx].sets[setIdx];
  pill.classList.add('editing');
  pill.innerHTML = `
    <input class="set-pill-edit-input" type="number" inputmode="decimal" step="0.5" value="${s.w}" data-ew />
    <span class="x">×</span>
    <input class="set-pill-edit-input" type="number" inputmode="numeric" value="${s.r}" data-er />
    <button class="set-pill-edit-ok" data-eok>OK</button>
  `;
  const wEl = pill.querySelector('[data-ew]');
  const rEl = pill.querySelector('[data-er]');
  wEl.focus();
  wEl.select();
  const commit = () => {
    ...
    if (isNaN(w) || isNaN(r) || r <= 0) { alert('Enter valid weight and reps'); return; }
    live.exercises[exIdx].sets[setIdx] = { w, r };
    saveLive();
    renderLive();
  };
  pill.querySelector('[data-eok]').addEventListener('click', (e) => { e.stopPropagation(); commit(); });
  rEl.addEventListener('keydown', (e) => { if (e.key === 'Enter') commit(); });
  wEl.addEventListener('keydown', (e) => { if (e.key === 'Enter') rEl.focus(); });
}

// Drag-and-drop reorder for live exercises
let dragSrcIdx = null;
function wireDragReorder() {
  const items = document.querySelectorAll('.live-ex');
  items.forEach(el => {
    el.addEventListener('dragstart', (e) => {
      dragSrcIdx = +el.dataset.exIdx;
    ...
    el.addEventListener('dragend', () => {
      ...
      document.querySelectorAll('.live-ex').forEach(x => x.classList.remove('drag-over'));
    });
    el.addEventListener('dragover', (e) => {
      e.preventDefault();
      e.dataTransfer.dropEffect = 'move';
      const tgt = +el.dataset.exIdx;
      document.querySelectorAll('.live-ex').forEach(x => x.classList.remove('drag-over'));
      if (tgt !== dragSrcIdx) el.classList.add('drag-over');
    });
    el.addEventListener('drop', (e) => {
      e.preventDefault();
      const tgt = +el.dataset.exIdx;
      if (dragSrcIdx === null || dragSrcIdx === tgt) return;
      reorderExercise(dragSrcIdx, tgt);
      dragSrcIdx = null;
    });
  });

  // Touch-based reorder via the drag handle (HTML5 DnD is unreliable on mobile)
  document.querySelectorAll('.drag-handle').forEach(handle => {
    handle.addEventListener('touchstart', (e) => {
      e.preventDefault();
      const exEl = handle.closest('.live-ex');
      if (!exEl) return;
      touchDragIdx = +handle.dataset.drag;
      exEl.classList.add('dragging');
    ...
    handle.addEventListener('touchmove', (e) => {
      ...
      const overEx = over && over.closest ? over.closest('.live-ex') : null;
      document.querySelectorAll('.live-ex').forEach(x => x.classList.remove('drag-over'));
      if (overEx && +overEx.dataset.exIdx !== touchDragIdx) overEx.classList.add('drag-over');
    }, { passive: false });
    ...
    handle.addEventListener('touchend', (e) => {
      ...
      const over = document.elementFromPoint(touch.clientX, touch.clientY);
      const overEx = over && over.closest ? over.closest('.live-ex') : null;
      document.querySelectorAll('.live-ex').forEach(x => { x.classList.remove('drag-over'); x.classList.remove('dragging'); });
      if (overEx) {
        const tgt = +overEx.dataset.exIdx;
        if (tgt !== touchDragIdx) {
          reorderExercise(touchDragIdx, tgt);
          touchDragIdx = null;
          return;
        }
      }
...
function reorderExercise(from, to) {
  const arr = live.exercises;
  const [moved] = arr.splice(from, 1);
  arr.splice(to, 0, moved);
  // Keep currentExIdx pointing at the same exercise
  if (live.currentExIdx === from) live.currentExIdx = to;
  else if (from < live.currentExIdx && to >= live.currentExIdx) live.currentExIdx--;
  else if (from > live.currentExIdx && to <= live.currentExIdx) live.currentExIdx++;
  saveLive();
  renderLive();
}
...
// ============================================================
// Exercise picker modal — all past exercises grouped by muscle
// ============================================================
...
function closeExercisePicker() {
  document.getElementById('exPickerModal').classList.remove('show');
  pickerCallback = null;
}
function renderPickerBody(filter) {
  filter = (filter || '').toLowerCase();
  // Gather every unique exercise from history, with last-used date and count
  const exMap = new Map();
  const sorted = [...sessions].sort((a,b) => b.date.localeCompare(a.date));
  for (const s of sorted) {
    for (const ex of s.exercises) {
      const k = norm(ex.name);
      if (!exMap.has(k)) exMap.set(k, { name: ex.name, lastDate: s.date, count: 1 });
      else exMap.get(k).count++;
    }
  }
  // Exercises already in the live session — mark them
  const inSession = new Set(live.exercises.map(e => norm(e.name)));

  let items = [...exMap.values()].filter(e => e.name.toLowerCase().includes(filter));

  // Group by primary muscle
  const groups = {};
  for (const it of items) {
    const muscles = musclesFor(it.name);
    const primary = muscles.length ? muscles[0] : 'Other';
    if (!groups[primary]) groups[primary] = [];
    groups[primary].push(it);
  }
  const groupOrder = [...MUSCLE_ORDER, 'Other'];
  let html = '';
  for (const g of groupOrder) {
    if (!groups[g] || !groups[g].length) continue;
    groups[g].sort((a,b) => b.lastDate.localeCompare(a.lastDate));
    html += `<div class="picker-group-label">${g}</div>`;
    html += groups[g].map(it => {
      const already = inSession.has(norm(it.name));
      return `<div class="picker-item" data-pick="${it.name.replace(/"/g, '&quot;')}">
        <span class="picker-item-name">${it.name}${already ? ' <span style="color:var(--accent-dim)">· in session</span>' : ''}</span>
        <span class="picker-item-meta">${it.count}× · ${it.lastDate.slice(5)}</span>
      </div>`;
    }).join('');
  }
  if (!html) html = '<div class="empty">No matching exercises</div>';
  // Custom-add row always available
  html += `<div class="picker-custom">
    <input type="text" id="pickerCustomInput" placeholder="Or type a new exercise…" value="${filter && !items.length ? filter.replace(/"/g, '&quot;') : ''}" />
    <button id="pickerCustomBtn">Add</button>
  </div>`;
  document.getElementById('exPickerBody').innerHTML = html;

  document.querySelectorAll('[data-pick]').forEach(el => {
    el.addEventListener('click', () => {
      if (pickerCallback) pickerCallback(el.dataset.pick);
      closeExercisePicker();
    });
  });
  const customBtn = document.getElementById('pickerCustomBtn');
...
// Exercise picker modal wiring
document.getElementById('exPickerClose').addEventListener('click', closeExercisePicker);
document.getElementById('exPickerModal').addEventListener('click', (e) => {
  if (e.target.id === 'exPickerModal') closeExercisePicker();
});
document.getElementById('exPickerSearch').addEventListener('input', (e) => renderPickerBody(e.target.value));
